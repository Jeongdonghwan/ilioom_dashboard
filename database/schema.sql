...
-- ========================================
-- 5. 사용자별 누적 광고 데이터 테이블
-- ========================================
CREATE TABLE IF NOT EXISTS ad_accumulated_data (
    id INT PRIMARY KEY AUTO_INCREMENT COMMENT '데이터 ID',
    user_id VARCHAR(20) NOT NULL COMMENT '사용자 ID',
    date DATE NOT NULL COMMENT '날짜',
    campaign_name VARCHAR(255) NOT NULL COMMENT '캠페인명',
    spend DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '지출액 (원)',
    impressions INT DEFAULT 0 COMMENT '노출수',
    clicks INT DEFAULT 0 COMMENT '클릭수',
    conversions INT DEFAULT 0 COMMENT '전환수',
    revenue DECIMAL(12, 2) DEFAULT 0 COMMENT '매출액 (원)',
    ad_type VARCHAR(20) DEFAULT 'general' COMMENT '광고 유형 (general 또는 coupang)',
    uploaded_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '업로드 일시',

    -- 유니크 제약 (같은 사용자+날짜+캠페인+유형이면 덮어쓰기)
    UNIQUE KEY uk_user_date_campaign (user_id, date, campaign_name, ad_type),

    -- 인덱스
    INDEX idx_user_date (user_id, date) COMMENT '사용자별 날짜 검색',
    INDEX idx_date_range (user_id, date, ad_type) COMMENT '날짜 범위 + 유형 필터'

) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='사용자별 누적 광고 데이터 - 스냅샷 없이 독립 저장, UPSERT 방식';
